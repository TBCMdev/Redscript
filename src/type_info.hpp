...
struct rs_type_info
{
    ...
    inline rs_type_info element_type() const
    {
        ...
        return rs_type_info{type_id, array_count - 1, optional, strict, generic, generic_id, otherTypes};
    }
    inline bool equals(const rs_type_info& other) const
    {
        const bool aeq = array_count == other.array_count;
    ...
    static inline void resolveGenericsIn(rs_type_info& info, const std::vector<rs_type_info>& generics)
    {
        if (info.generic && info.generic_id >= 0 && info.generic_id < (int)generics.size())
            info = generics[info.generic_id];
